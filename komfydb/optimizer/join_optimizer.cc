...
struct Edge {
  Edge(int join_idx, int table_idx)
      : join_idx(join_idx), table_idx(table_idx) {}
...
struct JoinInfo {
  JoinInfo(int cost, int size, int performed_join)
      : cost(cost), size(size), performed_join(performed_join) {}
...
JoinInfo join_cost(int set_number, int element_number,
                   std::vector<JoinNode>& joins,
                   std::vector<std::vector<Edge>>& tree,
                   std::vector<int>& predicted_size) {
...
absl::Status JoinOptimizer::OrderJoins(std::vector<JoinNode>& joins,
  ...
  for (int set_number = 0; set_number < number_of_subsets; set_number++) {
    // TODO (?)abstract popcount to separate function
    if (std::__popcount(set_number) == 1) {
      ...
      predicted_size[set_number] =
          table_stats_map[table_name]->EstimateTableCardinality();
    } else {
      int best_to_add = -1;
      ...
      for (int i = 0; i < N; i++) {
        if (element_in_set(i, set_number)) {
          ...
          auto [cost, size, performed_join] = join_cost(
              subset_number, element_number, joins, tree, predicted_size);
          if (cost == -1) {
            continue;
          }
          int total_cost =
              optjoin[subset_number] + optjoin[element_number] + cost;
  ...
  std::vector<JoinNode> ordered_joins;
  ...
  joins.clear();
  for(int i = 0; i < ordered_joins.size(); i++){
    joins.push_back(std::move(ordered_joins[ordered_joins.size()-i-1]));
  }
