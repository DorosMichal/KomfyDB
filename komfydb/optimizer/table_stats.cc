...
absl::StatusOr<TableStats> TableStats::Create(int table_id,
  ...
  // read whole table
  auto table_iterator = TableIterator::Create(TransactionId(), table_id, catalog, bufferpool);
  auto open_status = table_iterator->Open();
  if(!open_status.ok()){
    LOG(INFO) << "Failed to open table iterator to read statistics from table with id " << table_id;
    return open_status;
  }
  TableStats table_stats(table_id, io_cost_per_page);
  ...
  for(int i = 0; i < tuple_desc->Length(); i++){
    ...
    if(field_type.GetValue() == Type::Value::INT){
      table_stats.histograms.push_back(std::make_unique<IntHistogram>(records, i));
    } else {
      table_stats.histograms.push_back(std::make_unique<StringHistogram>(records, i));
    }
  }

  // get table size
  table_stats.number_of_tuples = records.size();
  table_stats.number_of_pages = catalog->GetDatabaseFile(table_id)->PageCount();
...
int TableStats::EstimateTableCardinality() {
  double selectivity = filter_information.filter_present ? filter_information.compound_selectivity : 1.0;
