...
StringHistogram::StringHistogram(std::vector<std::unique_ptr<Record>>& records,
                                 int field_index) {
  number_of_values = records.size();
  for (int i = 0; i < number_of_values; i++) {
    StringField* field = (StringField*)*records[i]->GetField(field_index);
...
double StringHistogram::EstimateSelectivity(execution::Op op, Field* constant) {
  ...
  switch (op.value) {
    ...
    case execution::Op::GREATER_THAN_OR_EQ:
      return (double)(number_of_values - number_of_smaller_equal) /
             number_of_values;
...
double StringHistogram::AverageSelectivity() {
  // TODO
