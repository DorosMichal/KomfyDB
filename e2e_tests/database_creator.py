...
class DatabaseCreator():
    ...
    def generate_values(self):
            ...
            for name, field in fields.items():
                ...
                match generation_type:
                    case ["range", start, stop]:
                        values = type_map[field_type][GenerationType.RANGE](size, start, stop)
                    case ["range", start, stop, number]:
                        values = type_map[field_type][GenerationType.RANGE](size, start, stop, number)
                    case ["unique"]:
                        values = type_map[field_type][GenerationType.UNIQUE](size)
                    case ["foreign", source_table_name, source_field_name]:
                        foreign_fields.append(ForeignField(
                            table, name, field_type, size, source_table_name, source_field_name
                        ))
                    case [choices] if isinstance(choices, list):
                        values = type_map[field_type][GenerationType.ENUM](size, choices)
                    case _:
        ...
        for field in foreign_fields:
            primary_keys_values = self.get_values(field.source_table_name, field.source_field_name)
            values = type_map[field.field_type][GenerationType.FOREIGN](field.size, primary_keys_values)
    ...
    def create_table(self, table_name, column_names):
        field_descriptions = [f"{column_name} {self.get_type(table_name, column_name).value}" for column_name in column_names]
